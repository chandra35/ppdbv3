...
class RekapNilaiExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnWidths, WithTitle, ShouldAutoSize
{
    protected $jalurId;
    ...
    /**
     * Preload semua data nilai agar tidak N+1 query
     */
    private function preloadData()
    {
        ...
        $this->nilaiSeleksiMap = $seleksiQuery->get()->keyBy('calon_siswa_id');
        ...
        $this->nilaiCbtMap = $cbtQuery->get()->keyBy('calon_siswa_id');
        ...
        $this->raporMap = $raporQuery->groupBy('calon_siswa_id')
    ...
    public function map($pendaftar): array
    {
        ...
        // Get rata-rata rapor
        $avgRapor = $this->raporMap[$pendaftar->id] ?? '';

        // Get TBQ data
        $tbq = $this->nilaiSeleksiMap[$pendaftar->id] ?? null;

        // Get CBT data
        $cbt = $this->nilaiCbtMap[$pendaftar->id] ?? null;

        // Hitung nilai akhir: CBT 50% + Rapor 10% + TBQ 40% (proporsional)
        $nilaiCbtRata = $cbt ? (float) $cbt->rata_rata : null;
        $nilaiRaporRata = $avgRapor !== '' ? (float) $avgRapor : null;
        $nilaiTbqTotal = $tbq && $tbq->total_nilai > 0 ? (float) $tbq->total_nilai : null;

        $totalBobot = 0;
        $totalNilai = 0;
        if ($nilaiCbtRata !== null) {
            $totalNilai += $nilaiCbtRata * 50;
            $totalBobot += 50;
        }
        if ($nilaiRaporRata !== null) {
            $totalNilai += $nilaiRaporRata * 10;
            $totalBobot += 10;
        }
        if ($nilaiTbqTotal !== null) {
            $totalNilai += $nilaiTbqTotal * 40;
            $totalBobot += 40;
        }
        $nilaiAkhir = $totalBobot > 0 ? round($totalNilai / $totalBobot, 2) : '';
